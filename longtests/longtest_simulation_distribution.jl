...
Uses flexsurv parameterization: h(t) = rate * exp(shape * t)
                                H(t) = (rate/shape) * (exp(shape*t) - 1)
"""
struct GompertzReference <: ContinuousUnivariateDistribution
    rate::Float64   # renamed from scale for clarity
    shape::Float64
end
...
function Distributions.cdf(dist::GompertzReference, x::Real)
    ...
    if abs(dist.shape) < 1e-10
        # Limit as shape → 0: exponential with rate = dist.rate
        return 1 - exp(-dist.rate * xval)
    else
        exp_term = _gompertz_exp_term(dist, xval)
        # H(t) = (rate/shape) * (exp(shape*t) - 1)
        cumhaz = (dist.rate / dist.shape) * (exp_term - 1)
        return 1 - exp(-cumhaz)
    end
end
...
function Distributions.pdf(dist::GompertzReference, x::Real)
    ...
    xval = float(x)
    if abs(dist.shape) < 1e-10
        return dist.rate * exp(-dist.rate * xval)
    else
        exp_term = _gompertz_exp_term(dist, xval)
        # h(t) = rate * exp(shape * t)
        # S(t) = exp(-H(t)) = exp(-(rate/shape) * (exp(shape*t) - 1))
        cumhaz = (dist.rate / dist.shape) * (exp_term - 1)
        return dist.rate * exp_term * exp(-cumhaz)
    end
end
...
function Distributions.quantile(dist::GompertzReference, p::Real)
    ...
    p == 1.0 && return Inf
    if abs(dist.shape) < 1e-10
        return -log1p(-p) / dist.rate
    else
        # Solve: 1 - exp(-(rate/shape)*(exp(shape*t) - 1)) = p
        # (rate/shape)*(exp(shape*t) - 1) = -log(1-p)
        # exp(shape*t) = 1 + shape * (-log(1-p)) / rate
        # t = log(1 + shape * (-log(1-p)) / rate) / shape
        return log(1 + dist.shape * (-log1p(-p)) / dist.rate) / dist.shape
    end
end
...
function Distributions.mean(dist::GompertzReference)
    ...
    integrand(t) = begin
        cumhaz = (dist.rate / dist.shape) * (exp(dist.shape * t) - 1)
        exp(-cumhaz)
    end
...
function scenario_distribution(s::SimulationScenario)
    ...
    elseif s.family == :gom
        shape = s.baseline.shape
        rate = s.baseline.scale  # scale in config = rate in flexsurv
        if s.effect == :ph
            rate_eff = rate * exp(linpred)
            shape_eff = shape
        else
        ...
        return GompertzReference(rate_eff, shape_eff)
    else
        error("Unsupported family $(s.family)")
    end
end
