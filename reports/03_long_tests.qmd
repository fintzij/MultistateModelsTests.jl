...
Tests in `longtest_spline_suite.jl` validate round-trip spline hazard recovery using a 
spline-based data generating process. Data is simulated from a spline model with known 
coefficients, then a spline model is fitted and the estimated hazard curve is compared
to the true hazard curve at multiple time points.

::: {.callout-note}
## Validation Strategy

Unlike parametric tests that compare parameter values directly, spline tests compare 
**hazard curves** since B-spline basis coefficients are not directly comparable between
models with different data. Validation checks that pointwise hazard h(t) and covariate
effects β are recovered within tolerance.
:::
...
#| echo: false
#| output: asis
# Show results for available exact spline suite tests
for test_name in ["sp_ph_exact_nocov", "sp_ph_exact_fixed", "sp_ph_exact_tvc",
                  "sp_aft_exact_nocov", "sp_aft_exact_fixed", "sp_aft_exact_tvc"]
    result = load_longtest_result(test_name)
    if !isnothing(result)
        status = result.passed ? "✅ Pass" : "❌ Fail"
        println("- **$test_name**: $status")
    else
        println("- **$test_name**: Not available")
    end
end
```
