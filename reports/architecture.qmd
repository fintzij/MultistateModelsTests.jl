...
# Draw type boxes with CORRECT hierarchy - wider layout, narrower boxes
boxes = [
    # (x, y, label, width, abstract?)
    (10, 8, "_Hazard", 1.0, true),
    (5, 6, "_MarkovHazard", 1.4, true),
    (15, 6, "_SemiMarkovHazard", 1.8, true),
    (2.5, 4, "MarkovHazard", 1.3, false),
    (7.5, 4, "PhaseTypeCoxianHazard", 2.0, false),
    (12.5, 4, "SemiMarkovHazard", 1.6, false),
    (17.5, 4, "_SplineHazard", 1.3, true),
    (17.5, 2, "RuntimeSplineHazard", 1.8, false),
]
...
for (x, y, label, w, is_abstract) in boxes
    ...
    poly!(ax, 
        Point2f[(x-w, y-0.35), (x+w, y-0.35), (x+w, y+0.35), (x-w, y+0.35)],
...
arrow_coords = [
    (10, 7.65, 5, 6.35),       # _Hazard -> _MarkovHazard
    (10, 7.65, 15, 6.35),      # _Hazard -> _SemiMarkovHazard
    (5, 5.65, 2.5, 4.35),      # _MarkovHazard -> MarkovHazard
    (5, 5.65, 7.5, 4.35),      # _MarkovHazard -> PhaseTypeCoxianHazard
    (15, 5.65, 12.5, 4.35),    # _SemiMarkovHazard -> SemiMarkovHazard
    (15, 5.65, 17.5, 4.35),    # _SemiMarkovHazard -> _SplineHazard
    (17.5, 3.65, 17.5, 2.35),  # _SplineHazard -> RuntimeSplineHazard
]
...
# Legend
text!(ax, 1, 1.2, text="Abstract types (dashed)", fontsize=9, color=:gray50, align=(:left, :center))
text!(ax, 1, 0.6, text="Concrete types (solid)", fontsize=9, color=:steelblue, align=(:left, :center))
...
#| fig-width: 12
#| fig-height: 5
# Visualize Coxian phase-type structure with improved visibility
fig = Figure(size=(1100, 450), backgroundcolor=:white)
ax = Axis(fig[1, 1], limits=(-1, 15, -1, 6), backgroundcolor=:white)
hidedecorations!(ax)
hidespines!(ax)

# Define colors - high contrast
phase_fill = colorant"#4A90D9"      # Medium blue for phases
phase_stroke = colorant"#1E5799"    # Dark blue stroke
exit_fill = colorant"#E67E22"       # Orange for exit state
exit_stroke = colorant"#A04000"     # Dark orange stroke
arrow_phase = colorant"#2C3E50"     # Dark gray for phase arrows
arrow_exit = colorant"#C0392B"      # Red for exit arrows

# Phase circles (larger, more visible)
phase_radius = 0.7
phases = [(2, 3, "Phase 1"), (5.5, 3, "Phase 2"), (9, 3, "Phase 3")]
for (x, y, label) in phases
    poly!(ax, Circle(Point2f(x, y), phase_radius), 
          color=(phase_fill, 0.4), strokecolor=phase_stroke, strokewidth=3)
    text!(ax, x, y, text=label, fontsize=13, align=(:center, :center), 
          color=:black, font=:bold)
end

# Exit state (larger)
exit_x, exit_y = 12.5, 3
poly!(ax, Circle(Point2f(exit_x, exit_y), phase_radius), 
      color=(exit_fill, 0.4), strokecolor=exit_stroke, strokewidth=3)
text!(ax, exit_x, exit_y, text="State 2", fontsize=13, align=(:center, :center), 
      color=:black, font=:bold)

# Phase progression arrows (thick, dark)
arrows!(ax, [2.7, 6.2], [3, 3], [2.0, 2.0], [0, 0],
        color=arrow_phase, linewidth=3, arrowsize=15)

# Phase labels
text!(ax, 0.5, 3, text="State 1\n(expanded)", fontsize=12, align=(:center, :center), 
      color=phase_stroke, font=:bold)

# Exit arrows (curved paths using lines, red/orange)
# From Phase 1 to Exit
lines!(ax, [2.5, 7, 11.8], [3.6, 5, 3.5], color=arrow_exit, linewidth=2.5, linestyle=:dash)
scatter!(ax, [11.8], [3.5], marker=:rtriangle, markersize=12, color=arrow_exit, rotation=pi/6)

# From Phase 2 to Exit  
lines!(ax, [6.1, 9.5, 11.8], [3.4, 4.2, 3.3], color=arrow_exit, linewidth=2.5, linestyle=:dash)
scatter!(ax, [11.8], [3.3], marker=:rtriangle, markersize=12, color=arrow_exit, rotation=pi/12)

# From Phase 3 to Exit (direct)
lines!(ax, [9.7, 11.8], [3, 3], color=arrow_exit, linewidth=2.5, linestyle=:dash)
scatter!(ax, [11.8], [3], marker=:rtriangle, markersize=12, color=arrow_exit)

# Rate labels (larger, more visible)
text!(ax, 3.7, 3.5, text="λ₁", fontsize=16, color=arrow_phase, font=:bold)
text!(ax, 7.3, 3.5, text="λ₂", fontsize=16, color=arrow_phase, font=:bold)
text!(ax, 4.5, 5.2, text="μ₁", fontsize=14, color=arrow_exit, font=:bold)
text!(ax, 8.3, 4.5, text="μ₂", fontsize=14, color=arrow_exit, font=:bold)
text!(ax, 10.7, 2.5, text="μ₃", fontsize=14, color=arrow_exit, font=:bold)

# Legend box
Legend(fig[2, 1], 
    [LineElement(color=arrow_phase, linewidth=3),
     LineElement(color=arrow_exit, linewidth=2.5, linestyle=:dash)],
    ["λᵢ: Phase progression rate", "μᵢ: Exit rate to absorbing state"],
    orientation=:horizontal, framevisible=false, labelsize=12)

# Caption
Label(fig[3, 1], 
    "Coxian phase-type: State 1 has 3 latent phases. Exit to State 2 possible from any phase.\nAll transitions are exponential → process on expanded space is Markovian.",
...
p_natural = get_parameters(model; scale=:natural)  # Interpretable values
...
- Sampling Importance Resampling (SIR) for weight degeneracy
