...
if isfile(results_path)
    ...
    mcem_df = haskey(json_data, :mcem) ? json_to_df(json_data.mcem) : DataFrame(Method=[], Runtime=[])
    ...
    mcem_df = DataFrame(Method=[], Runtime=[])
...
#| echo: false
if !isempty(mcem_df)
    println("MCEM Runtime (N=200): $(round(mcem_df.Runtime[1], digits=4)) seconds")
else
    println("No MCEM data available.")
end
```
...
3. **Threading**: Parallel likelihood computation
4. **Phase-type proposals**: Critical for non-exponential hazards in MCEM
5. **SIR/LHS resampling**: Variance reduction for importance sampling

### Optimization Tips

1. **Use exact observations when available** - much faster than panel
2. **Match threads to physical cores** - avoid hyperthreading overhead
3. **Use phase-type proposals for semi-Markov** - automatic selection in `fit()`
4. **Profile before optimizing** - identify actual bottlenecks
5. **Use adaptive SIR** - automatically switches to resampling when beneficial
